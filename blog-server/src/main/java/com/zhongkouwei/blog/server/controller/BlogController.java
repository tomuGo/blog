...
@RestController
@RequestMapping(value = BlogConstants.BLOG_URL)
public class BlogController {
    ...
    public ResultSub<PageInfo<Blog>> getBlogs(@RequestParam("pageNumber") Integer pageNumber,
        ...
        PageInfo<Blog> blogs = blogService.getBlogs(pageable,criteria);
        return new ResultSub<>(blogs);
    }

    @RequestMapping(value = "blog/{id}/addSection", method = RequestMethod.POST)
    public ResultSub<Integer> addSection(@PathVariable("id") String blogId, @RequestBody Floor section) {
        Integer floorId=blogService.addSection(section, blogId);
